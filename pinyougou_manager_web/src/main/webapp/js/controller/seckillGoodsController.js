	...
	var app = new Vue({
		...
		methods:{
			...
			findPage:function (pageNo) {
				axios.post("../seckillGoods/findPage.do?pageNo="+pageNo+"&pageSize="+10,this.searchEntity)
			...
			//新增
			add:function () {
				var url = "../seckillGoods/add.do";
				if(this.entity.id != null){
					url = "../seckillGoods/update.do";
				}
			...
			//跟据id查询
			getById:function (id) {
				axios.get("../seckillGoods/getById.do?id="+id).then(function (response) {
					app.entity = response.data;
				})
			},
			//批量删除数据
			dele:function () {
				axios.get("../seckillGoods/delete.do?ids="+this.ids).then(function (response) {
					if(response.data.success){
						//刷新数据
						app.findPage(app.pageNo);
						//清空勾选的ids
